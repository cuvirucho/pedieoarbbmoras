...
const Checkout = ({ cart, setCart }) => {
  ...
  const total = cart.reduce(
    (acc, i) => acc + Number(i.precioVenta) * Number(i.cantidad),
    0,
  );
  ...
  const enviarPedido = async () => {
    ...
    try {
      const res = await fetch(`${API}/create-order`, {
        ...
        body: JSON.stringify({ cart, form, metodo, coords, delivery }),
      });
      ...
      container.innerHTML = "";
      window.ppbInstance = new window.PPaymentButtonBox(cfg).render(
        "pp-button",
      );
    } catch (err) {
  ...
  const costoEnvio = metodo === "hospedaje" ? 3.5 : 0;
  const iva = total * 0.18;
  const totalConIva = total + iva + costoEnvio;
  ...
  return (
    <>
                  ...
                  {coords ? (
                    <article className="contefulubu">
                      <div className="congtebucselc">
                        ...
                        <p className="direccion">{coords.address}</p>
                      </div>

                      <button
                        className="cambiar-ubicacion-btn"
                        onClick={muvopuntogps}
                      >
                        Cambiar ubicación
                      </button>
                    </article>
                  ...
                  {cart.map((i, idx) => (
                    <li key={idx} className="checkout-item">
                      ...
                      <span className="checkout-price">
                        ${(i.precioVenta * i.cantidad).toFixed(2)}
                      </span>
                    </li>
                  ))}
                </ul>
              ...
              {cart.length > 0 && (
                <div className="cart-footer2">
                  <div className="cart-total2">
                    ...
                    {metodo === "hospedaje" && (
                      <div className="row2">
                        <span className="label2">Envío:</span>
      ...
      {loading ? <p className="cargado">Cargando...</p> : null}
    </>
  );
};
